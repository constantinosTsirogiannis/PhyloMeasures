...
typedef Phylogenetic_Sorensens_similarity::Tree_type     Tree_type;
...
int main(int argc, char *argv[])
{
  ...
  {
    int rows = phylosor.csv_matrix_query_specific_pairs_basic(filename2, filename4, std::back_inserter(res));
    rows_cols = std::make_pair(rows,rows);     
  }
  else if(specific_pairs == true)
    rows_cols = phylosor.csv_matrix_query_specific_pairs_basic(filename2, filename3, 
                                                               filename4, std::back_inserter(res));
  else if(single_matrix == true)
  {
    int rows = phylosor.csv_matrix_query_basic(filename2, std::back_inserter(res));
        rows_cols = std::make_pair(rows,rows);
  }
  else 
    rows_cols = phylosor.csv_matrix_query_basic(filename2, filename3, std::back_inserter(res));
