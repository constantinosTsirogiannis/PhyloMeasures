...
typedef Community_distance::Tree_type                    Tree_type;
...
int main(int argc, char *argv[])
{
  ...
  if(standardised == true)
  {
    ...
    {
      int rows = cd.csv_matrix_query_specific_pairs_standardised(filename2, filename4, std::back_inserter(res));
      rows_cols = std::make_pair(rows,rows);
    }
    else if(specific_pairs == true)
      rows_cols = cd.csv_matrix_query_specific_pairs_standardised(filename2, filename3, 
                                                                 filename4, std::back_inserter(res));
    else if(single_matrix == true)
    {
      int rows = cd.csv_matrix_query_standardised(filename2, std::back_inserter(res));
      rows_cols = std::make_pair(rows,rows);
    }
    else 
      rows_cols = cd.csv_matrix_query_standardised(filename2, filename3, std::back_inserter(res));
  }
  else
  {
    if(single_matrix == true && specific_pairs == true)
    {
      int rows = cd.csv_matrix_query_specific_pairs_basic(filename2, filename4, std::back_inserter(res));
      rows_cols = std::make_pair(rows,rows);     
    }
    else if(specific_pairs == true)
      rows_cols = cd.csv_matrix_query_specific_pairs_basic(filename2, filename3, 
                                                           filename4, std::back_inserter(res));
    else if(single_matrix == true)
    {
      int rows = cd.csv_matrix_query_basic(filename2, std::back_inserter(res));
      rows_cols = std::make_pair(rows,rows);
    }
    else 
      rows_cols = cd.csv_matrix_query_basic(filename2, filename3, std::back_inserter(res));
