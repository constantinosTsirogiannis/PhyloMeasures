...
typedef Common_branch_length::Tree_type                  Tree_type;
...
int main(int argc, char *argv[])
{
  ...
  if(standardised == true)
  {
    ...
    {
      int rows = cbl.csv_matrix_query_specific_pairs_standardised(filename2, filename4, std::back_inserter(res));
      rows_cols = std::make_pair(rows,rows);
    }
    else if(specific_pairs == true)
      rows_cols = cbl.csv_matrix_query_specific_pairs_standardised(filename2, filename3, 
                                                                   filename4, std::back_inserter(res));
    else if(single_matrix == true)
    {
      int rows = cbl.csv_matrix_query_standardised(filename2, std::back_inserter(res));
      rows_cols = std::make_pair(rows,rows);
    }
    else 
      rows_cols = cbl.csv_matrix_query_standardised(filename2, filename3, std::back_inserter(res));
  }
  else
  {
    if(single_matrix == true && specific_pairs == true)
    {
      int rows = cbl.csv_matrix_query_specific_pairs_basic(filename2, filename4, std::back_inserter(res));
      rows_cols = std::make_pair(rows,rows);
    }
    else if(specific_pairs == true)
      rows_cols = cbl.csv_matrix_query_specific_pairs_basic(filename2, filename3, 
                                                            filename4, std::back_inserter(res));
    else if(single_matrix == true)
    {
      int rows = cbl.csv_matrix_query_basic(filename2, std::back_inserter(res));
      rows_cols = std::make_pair(rows,rows);
    }
    else 
      rows_cols = cbl.csv_matrix_query_basic(filename2, filename3, std::back_inserter(res));
  } // else of if (standardised == true)
