...
typedef Community_distance_nearest_taxon::Tree_type      Tree_type;

int main(int argc, char *argv[])
{
  char *filename1, *filename2, *filename3, *filename4;
  std::pair<int,int> rows_cols;
  std::vector<double> res, res2;
  Tree_type tree;

  if(argc < 5 || argc > 9)
  {
    std::cout << std::endl << " Wrong number of arguments, should be between four and eight ... aborting. " 
              << std::endl << std::endl;
    return -1;
  } 
  ...
  for(int i=1; i<argc; i++)
    ...
    else if(std::string(argv[i]) == std::string("-maximised"))
    {
      mode_flag_count++;
      maximised = true;
    }
    else if(std::string(argv[i]) == std::string("-directed"))
    {
      mode_flag_count++;
      maximised = false; 
      directed = true;
    }
    else if(std::string(argv[i]) == std::string("-averaged"))
    {
      mode_flag_count++;
      maximised = false; 
      averaged = true;
  ...
  if(maximised == true)
  {
    ...
    {
      int rows = cdnt.csv_matrix_query_specific_pairs_basic(filename2, filename4, std::back_inserter(res));
      rows_cols = std::make_pair(rows,rows);
    }
    else if(specific_pairs == true)
      rows_cols = cdnt.csv_matrix_query_specific_pairs_basic(filename2, filename3, 
                                                             filename4, std::back_inserter(res));
    else if(single_matrix == true)
    {
      int rows = cdnt.csv_matrix_query_basic(filename2, std::back_inserter(res));
      rows_cols = std::make_pair(rows,rows);
    }
    else 
      rows_cols = cdnt.csv_matrix_query_basic(filename2, filename3, std::back_inserter(res));
  }
  else if(averaged == true)
  {
    if(single_matrix == true && specific_pairs == true)
    {
      int rows = cdnt.csv_matrix_query_averaged_specific_pairs_basic(filename2, filename4, 
                                                                             std::back_inserter(res));
      rows_cols = std::make_pair(rows,rows);
    }
    else if(specific_pairs == true)
      rows_cols = cdnt.csv_matrix_query_averaged_specific_pairs_basic(filename2, filename3, 
                                                                      filename4, std::back_inserter(res));
    else if(single_matrix == true)
    {
      int rows = cdnt.csv_matrix_query_averaged_basic(filename2, std::back_inserter(res));
      rows_cols = std::make_pair(rows,rows);
    }
    else 
      rows_cols = cdnt.csv_matrix_query_averaged_basic(filename2, filename3, std::back_inserter(res));
  } 
  else if(directed == true)
  {
    if(single_matrix == true && specific_pairs == true)
    {
      int rows = cdnt.csv_matrix_query_directed_specific_pairs_basic(filename2, filename4, 
                                                                     std::back_inserter(res),
                                                                     std::back_inserter(res2));
      rows_cols = std::make_pair(rows,rows);
    }
    else if(specific_pairs == true)
      rows_cols = cdnt.csv_matrix_query_directed_specific_pairs_basic(filename2, filename3, 
                                                                      filename4, std::back_inserter(res),
                                                                      std::back_inserter(res2));
    else if(single_matrix == true)
    {
      int rows = cdnt.csv_matrix_query_directed_basic(filename2, std::back_inserter(res));
      rows_cols = std::make_pair(rows,rows);
    }
    else 
      rows_cols = cdnt.csv_matrix_query_directed_basic(filename2, filename3, 
                                                       std::back_inserter(res), std::back_inserter(res2));
  } // else of if (maximised == true)

  if( maximised == true || averaged == true )
  {
  ...
  else
  {
    ...
    else
    {
      ...
      if(single_matrix == false && std::string(filename2) != std::string(filename3))
      {
        std::cout << std::endl;

        for( int i=0; i<rows_cols.second; i++ )
        {
          for( int j=0; j<rows_cols.first; j++ )
            std::cout << res2[(i*rows_cols.first) +j] << " "; 
    ...
    } // else of if(specific_pairs == true)

  } // else of if( maximised == true || averaged == true )
